...
type Tool = "calibrate" | "measure" | "chain";
type Unit = "mm" | "cm" | "m";
...
function drawArchitecturalDimension(
  ...
  for (const p of [start, end]) {
    ctx.beginPath();
    ctx.moveTo(p.x - nx * extension, p.y - ny * extension);
    ctx.lineTo(p.x + nx * extension, p.y + ny * extension);
    ctx.stroke();
...
function CropDialog({ candidate, onCancel, onUseOriginal, onCrop }: {
  ...
  return <div className="crop-overlay" role="dialog" aria-label="导入图纸">
    <div className="crop-dialog">
      ...
      <div className={`crop-preview ${mode ? "is-cropping" : ""}`}><canvas ref={canvasRef}
        onPointerDown={(event) => { if (!mode) return; event.currentTarget.setPointerCapture(event.pointerId); const p = point(event); startRef.current = p; setRect({x:p.x,y:p.y,w:0,h:0}); }}
        onPointerMove={(event) => { if (!mode || !startRef.current) return; const p = point(event), s = startRef.current; setRect({x:Math.min(s.x,p.x),y:Math.min(s.y,p.y),w:Math.abs(p.x-s.x),h:Math.abs(p.y-s.y)}); }}
        onPointerUp={() => { startRef.current = null; }} /></div>
      <div className="crop-footer"><span>{candidate.image.naturalWidth} × {candidate.image.naturalHeight} px{mode && rect.w > 2 ? `　→　${Math.round(rect.w)} × ${Math.round(rect.h)} px` : ""}</span><div>{mode ? <><button className="button ghost" onClick={() => setMode(false)}>返回</button><button className="button primary" disabled={rect.w < 10 || rect.h < 10} onClick={() => onCrop(rect)}>应用裁切</button></> : <><button className="button ghost" onClick={onUseOriginal}>直接使用原图</button><button className="button primary" onClick={() => setMode(true)}>裁切范围</button></>}</div></div>
    </div>
  </div>;
}
...
export default function Home() {
  ...
  const updateFit = useCallback(() => {
    ...
    setFitScale(Math.min(1, Math.max(240, r.width - 100) / plan.naturalWidth, Math.max(200, r.height - 100) / plan.naturalHeight));
  }, [plan]);
  ...
  const renderCanvas = useCallback(() => {
    ...
    const canvas = canvasRef.current;
    canvas.width = plan.naturalWidth; canvas.height = plan.naturalHeight;
    const ctx = canvas.getContext("2d"); if (!ctx) return;
    ctx.drawImage(plan, 0, 0);
    const width = Math.max(1.5, Math.min(canvas.width, canvas.height) / 1000 * 1.8);
    if (calibration) drawArchitecturalDimension(ctx, calibration, "校准基准", "#eb7b42", width, true);
    if (scaleMmPerPixel) measurements.forEach(line => drawArchitecturalDimension(ctx, line, formatLength(dist(line.start,line.end)*scaleMmPerPixel,displayUnit), "#b82933", width));
    if (activeStart && hoverPoint) {
      const draft = {id:"draft",start:activeStart,end:hoverPoint};
      const label = tool === "calibrate" ? "指定第二点" : scaleMmPerPixel ? formatLength(dist(activeStart,hoverPoint)*scaleMmPerPixel,displayUnit) : "指定第二点";
      drawArchitecturalDimension(ctx,draft,label,"#0b8d7b",width,true);
    }
  }, [plan, calibration, measurements, scaleMmPerPixel, displayUnit, activeStart, hoverPoint, tool]);
  useEffect(() => renderCanvas(), [renderCanvas]);

  const commitPlan = useCallback((image: HTMLImageElement, imageFile: File) => {
    setPlan(image); setFile(imageFile); setCandidate(null); setCalibration(null); setMeasurements([]); setScaleMmPerPixel(null); setActiveStart(null); setTool("calibrate"); setZoom(1); setPan({x:0,y:0}); setToast("图纸已载入：R 校准比例，F8 正交，F3 对象吸附");
  }, []);
  ...
  const rawPoint = (event: ReactPointerEvent<HTMLCanvasElement>): Point => {
    ...
    return {x:(event.clientX-r.left)*event.currentTarget.width/r.width,y:(event.clientY-r.top)*event.currentTarget.height/r.height};
  };
  ...
  const completeLine = (line: Line) => {
    if (tool === "calibrate") { setPendingCalibration(line); setActiveStart(null); return; }
    setMeasurements(items=>[...items,line]);
    if(tool==="chain")setActiveStart(line.end); else setActiveStart(null);
  };

  const onCanvasPointerDown = (event: ReactPointerEvent<HTMLCanvasElement>) => {
    if(event.button===1||spaceRef.current)return;
    if(event.button!==0||!plan||(tool!=="calibrate"&&!scaleMmPerPixel))return;
    ...
    else if(dist(activeStart,resolved.point)>2)completeLine({id:crypto.randomUUID(),start:activeStart,end:resolved.point});
  };
  ...
  const switchTool=useCallback((next:Tool)=>{ if(next!=="calibrate"&&!scaleMmPerPixel)return; setTool(next);setActiveStart(null);setSnap(null); },[scaleMmPerPixel]);
  ...
  useEffect(()=>{
    const down=(event:KeyboardEvent)=>{
      ...
      if(event.key==="Escape"){setActiveStart(null);setPendingCalibration(null);setSnap(null);}
      if(event.key==="Enter"&&tool==="chain")setActiveStart(null);
  ...
  const exportImage=()=>{if(!plan||!file)return;const output=document.createElement("canvas");output.width=plan.naturalWidth;output.height=plan.naturalHeight;const ctx=output.getContext("2d");if(!ctx)return;const mime=["image/png","image/jpeg","image/webp"].includes(file.type)?file.type:"image/png";if(mime==="image/jpeg"){ctx.fillStyle="#fff";ctx.fillRect(0,0,output.width,output.height);}ctx.drawImage(plan,0,0);const width=Math.max(1.5,Math.min(output.width,output.height)/1000*1.8);if(includeCalibration&&calibration)drawArchitecturalDimension(ctx,calibration,"校准基准","#eb7b42",width,true);if(scaleMmPerPixel)measurements.forEach(line=>drawArchitecturalDimension(ctx,line,formatLength(dist(line.start,line.end)*scaleMmPerPixel,displayUnit),"#b82933",width));output.toBlob(blob=>{if(!blob)return;const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`${file.name.replace(/\.[^.]+$/,"")}-已标注.${mime==="image/jpeg"?"jpg":mime.split("/")[1]}`;a.click();URL.revokeObjectURL(url);},mime,.95);};

  const draftCommand=activeStart?(tool==="chain"?"连续标注：指定下一点（Enter 结束）":"指定第二条尺寸界线原点"):tool==="calibrate"?"校准比例：指定第一点":tool==="chain"?"连续标注：指定起点":"线性标注：指定第一条尺寸界线原点";
  const snapScreen=useMemo(()=>{if(!snap||!plan)return null;const scale=fitScale*zoom;return{x:stageSize.w/2+pan.x-plan.naturalWidth*scale/2+snap.point.x*scale,y:stageSize.h/2+pan.y-plan.naturalHeight*scale/2+snap.point.y*scale};},[snap,plan,fitScale,zoom,pan,stageSize]);
  ...
  return <main className="cad-app">
    ...
    <section className="cad-workspace">
      ...
      <aside className="cad-properties"><div className="prop-title"><span>特性</span><strong>{tool==="calibrate"?"比例校准":tool==="chain"?"连续标注":"线性标注"}</strong></div><section><h3>图纸</h3><dl><div><dt>文件</dt><dd>{file?.name||"—"}</dd></div><div><dt>像素</dt><dd>{plan?`${plan.naturalWidth} × ${plan.naturalHeight}`:"—"}</dd></div><div><dt>比例</dt><dd className={scaleMmPerPixel?"ok":""}>{scaleMmPerPixel?`1 px = ${scaleMmPerPixel.toFixed(3)} mm`:"未校准"}</dd></div></dl></section><section><h3>尺寸样式</h3><label className="prop-field"><span>显示单位</span><select value={displayUnit} onChange={e=>setDisplayUnit(e.target.value as Unit)}><option>mm</option><option>cm</option><option>m</option></select></label><label className="prop-check"><input type="checkbox" checked={includeCalibration} onChange={e=>setIncludeCalibration(e.target.checked)}/>导出校准线</label></section><section><h3>对象捕捉</h3><p>端点 · 中点 · 交点 · 水平/垂直对齐</p><small>当前吸附已标注的几何点。图片墙线自动识别将在下一阶段加入。</small></section><section className="shortcut-card"><h3>快捷键</h3><dl><div><dt>R / D / C</dt><dd>校准 / 单段 / 连续</dd></div><div><dt>F8 / F3</dt><dd>正交 / 对象捕捉</dd></div><div><dt>滚轮</dt><dd>指针中心缩放</dd></div><div><dt>中键 / 空格</dt><dd>平移画布</dd></div><div><dt>Esc / Enter</dt><dd>取消 / 结束连续</dd></div></dl></section></aside>
